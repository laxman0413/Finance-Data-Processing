...
1. A newly registered user always gets the `viewer` role. Admins can promote users.
2. Financial records are owned by the user who created them, but all authenticated users with sufficient role can read any record.
3. An admin cannot delete their own account via the users endpoint.
4. Pagination defaults to 20 records per page.
5. Monthly trends cover the past 12 months from the current date.
6. `date` fields in financial records are stored as `YYYY-MM-DD` strings.
